	...
	if (!req.session.authenticated) {
		...
		MongoClient.connect(mongourl, function(err, db) {
			...
			db.collection('restaurant').findOne({name: req.session.name},function(err,doc) {
				...
				res.status(200);
				res.render('restaurant',{name:doc.name, borough:doc.borough,cuisine:doc.cuisine,username:doc.grade.username,score:doc.grade.score, owner:doc.owner, building:doc.address.building, street:doc.address.street, lat: doc.address.lat, long:doc.address.long});
			});
		});
	}
});
...
function update(req,res,callback) {
	console.log('test');
	var u = {};	// document to be inserted
	if (req.body.name) u['name'] = req.body.name;
	if (req.body.borough) u['borough'] = req.body.borough;
	if (req.body.cuisine) u['cuisine'] = req.body.cuisine;
	if (req.body.building || req.body.street || req.body.lat || req.body.long) {
		var address = {};
		if (req.body.building) address['building'] = req.body.building;
		if (req.body.street) address['street'] = req.body.street;
	...
	console.log('About to insert: ' + JSON.stringify(u));
	MongoClient.connect(mongourl,function(err,db) {
		assert.equal(err,null);
		console.log('Connected to MongoDB\n');
		updateRestaurant(req,db,u,function(result) {
			db.close();
			console.log(JSON.stringify(result));
			callback(cb);
		});
